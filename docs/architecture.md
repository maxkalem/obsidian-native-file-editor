...
## Unwrap decides a hyphen by the text first, the word lists second

`fmt/unwrap.ts` joins hard-wrapped prose back into paragraphs, and the one judgement it cannot make from line lengths is the hyphen at a line end: the printer's, which goes, or the word's own, which stays. The order is fixed and the first source that answers wins. The evidence of the document (`buildTextEvidence`, one pass over the words of the whole file, so a selection is judged by the file around it): the intact `a-b` or the joined `ab` written elsewhere, the more frequent winning, then a part that begins or ends at least two other hyphenated words. Then the lexicon of the dictionaries (`fmt/dictionary.ts` merges the bundled lists of `fmt/textLanguages.ts` with the vault's). Then what holds in any language: the parts repeat, one letter before the hyphen, an all-uppercase abbreviation, the same Latin vowel on both sides. Otherwise the hyphen was the printer's.

A script with no case, and a script with no spaces, both had to be taught. The gate and the join used to ask whether the next line starts with a LOWERCASE letter, which Arabic, Hebrew, Chinese, Japanese, Korean, Thai, Devanagari, Bengali and Tamil can never do: measured on one wrapped paragraph each (2026-09-18), every one of them was refused as "not-prose" or never measured at all, and Unwrap did nothing in those languages. A caseless letter (`\p{Lo}`) now counts as a continuation exactly as a lowercase one does. Lines are measured in COLUMNS, not characters, because a wrapper counted columns and an East Asian character takes two: 28 Chinese characters are a 56-column line, which the old count read as 28 and dropped below the minimum wrap width. And where a script writes without spaces between words (Han, kana, Thai, Lao, Khmer, Myanmar — not Korean, which uses spaces) the join puts no space between the two lines. Georgian needed nothing: Mkhedruli is `Ll`, with Mtavruli as its uppercase, so it worked from the start. The hyphen class holds the Armenian hyphen ֊ and the Hebrew maqaf ־ beside the ASCII one. A dry run over the vault's 3 836 notes before and after: the same 28 files change, byte for byte.
...
## One catalogue, one file over it

Every user-visible string goes through `t(key)` in `core/i18n.ts`, and the English text lives in `i18n/en.ts` as a flat map of dotted keys. English is what ships. One file, `localization.json` in the plugin's own folder, replaces it: `core/localization.ts` reads that one path once, at load. There is no folder of languages, no list, no setting and nothing to choose, because a person wants their own language rather than a menu of them; a changed file applies when the plugin restarts, which the Plugin group's Reload plugin does. No file, an empty file, or a file that does not parse: English, and only the last of those says anything.

The fallback is per key: a file that translates thirty lines shows English for the rest, so a translation is something a person starts rather than a project someone has to finish. Nothing is translated at module load: `t()` runs when the text is built, so the catalogue in force is always the one read at the last start. Three key families are built at run time — `hotkey.<id>.name`, `hotkey.<id>.meaning`, `keywordRole.<role>` — and only because those lists are fixed and the tests walk them. Log lines, parse errors and the transport's messages stay English: a log is a diagnostic that other people read.

Three tests hold this together. `tests/catalogue.test.ts` scans the source for keys and fails both on a key nothing defines and on a key nothing uses. `tests/locales.test.ts` reads the two example files in `locales/` and fails on an unknown key or a lost `{placeholder}`, and asserts that `locales/english.json` is still the catalogue as it stands (it is generated from `localizationExample`). `tests/i18n.test.ts` covers the loader and the fallback.
...
The feature is off by default behind a per-device toggle and starts only from the button, the command or its hotkey. The reasoning, the two corrections made on the way ("no sandbox for native interpreters", "not in the plugin folder") and the reviewer-facing account are in `docs/ADR-004-run-code-with-user-interpreters.md` and `docs/threat-model.md`.
