...
- **Interpreters do not live in the plugin folder.** It is inside the vault: a synced or Git-tracked vault would carry hundreds of megabytes, reinstalling the plugin wipes that folder, and paths differ between devices. Interpreter locations are device-local settings: bare command names resolved through `PATH`, or absolute paths, including one inside the vault at the user's own risk.
...
- Output reaches the panel as text through `setText`/`appendText`, stdout and stderr distinguished by class, with the exit code and wall time. The panel is a `pre`, not an editor. Its two views split by role: Output is the result (a rendered page, or stdout — rendered in the page frame when it is an SVG or HTML document, text otherwise), Log is the console (the command, `console.*`, stderr, stdout as text, a page's reports, the outcome).
