...
Program output is text, appended to a `pre` with `appendText`, stdout and stderr in separate classes, truncated at the cap with a marker; it is not written anywhere. One exception, deliberate: when a program's stdout (or what a sandbox script posts) is a whole SVG or HTML document, the panel renders it — in the same `sandbox="allow-scripts"` frame, with the same injected policy and an opaque origin, as a page opened from the vault, so a program can draw. Nothing in it can reach Obsidian, the vault or the network any more than a page can; a document cut by the output cap is shown as text.
