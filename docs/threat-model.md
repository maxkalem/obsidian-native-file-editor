...
"Open in browser" (the default for HTML and MHTML) and the folder buttons in settings go through Electron's `shell.openPath` (`src/platform/desktopShell.ts`): the operating system opens the file or folder with whatever the user has associated with it, which is what double-clicking it would do; the plugin passes a path and starts nothing itself. The native dialogs in settings (choose a folder, choose an interpreter) are Electron's own; a chosen folder must lie inside the vault.
