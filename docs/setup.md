...
## A Hunspell dictionary as the review after Unwrap

A spell checker's dictionary answers a question the plugin's own lists cannot: whether the word Unwrap made by dropping a hyphen is a word at all. Put the pair a spell checker ships — `<language>.dic` and `<language>.aff` of the same name, from Aegisub's `dictionaries\`, LibreOffice, Firefox or your distribution — into the same dictionary folder (`.obsidian/plugins/native-file-editor/dictionaries/`). The plugin bundles none, downloads none and reads them only when asked; the `.json` lists ignore them, so both kinds live in one folder.

After **Unwrap lines** has dropped at least one hyphen, a dialog offers to check those words — only those, and only in this file. It then names the ones the dictionary does not know, with the hyphen put back beside each, and **Restore the hyphen** writes that form back, as its own undo step. The book button on a row writes the form you chose into the plugin's own word lists, so the next Unwrap decides it without asking. A dictionary is evidence, not a verdict: a name or a rare word is unknown to it and still right.

The file is read in pieces and the reading stops as soon as every word is answered, so nothing of it is held: 8.5 MB and 335 359 entries (Ukrainian) take about a third of a second, and 62 118 entries (English) a tenth of that. The `.aff` file's own `SET` line decides the encoding, and its suffix and prefix rules are what make `попереднього` findable under `попередній`. What the reader does not do: compound rules, two suffixes on one word, ICONV replacements — a word those would explain is reported as unknown, which is why the dialog asks rather than corrects. The log's `[hunspell]` line names the file, the words, the candidate stems and how much was read.
