...
- The preview is highlighted only below 1 MB (`PREVIEW_HIGHLIGHT_MAX_BYTES`); above that it is plain text so it still renders at once. The number is a guess until a device measurement replaces it.
