...
const TIER4 = {
  ...
  // `makefile` as an extension is what obsidian-code-editor registers; a dotless `Makefile` has no extension in Obsidian (open item).
  makefile: { name: "Makefile", ext: ["mak", "mk", "makefile"] },
  ...
  txt2tags: { name: "txt2tags", ext: ["t2t"] },
...
for (const m of xml.matchAll(/<Language\s+([^>]*?)(?:\/>|>([\s\S]*?)<\/Language>)/g)) {
  const a = attrs(m[1]);
  const spec = TIER4[a.name] ?? EXAMPLES[a.name];
  if (!spec) continue;
  seen.add(a.name);
  const sets = [];
  ...
  (TIER4[a.name] ? out : examples).push({
    id: a.name,
    name: spec.name,
    extensions: spec.ext,
    caseInsensitive: spec.ci === true,
    commentLine: a.commentLine ? decode(a.commentLine) : null,
    commentStart: a.commentStart ? decode(a.commentStart) : null,
    commentEnd: a.commentEnd ? decode(a.commentEnd) : null,
    sets,
  });
}
const missing = [...Object.keys(TIER4), ...Object.keys(EXAMPLES)].filter((k) => !seen.has(k));
if (missing.length > 0) {
