...
Unwrap reads scripts without case and scripts without spaces: a caseless letter (Arabic, Hebrew, Chinese, Japanese, Korean, Thai, Devanagari, Bengali, Tamil) continues a line as a lowercase one does, lines are measured in columns so a wide character counts for two, and a join in a script that writes without word spaces puts none.
...
**The review after Unwrap.** A hyphen at a line end is the one thing a text cannot always settle, so a spell checker's dictionary can be asked about the words Unwrap put together by dropping one. Copy a `<language>.dic` and its `.aff` (Aegisub's, LibreOffice's, Firefox's — the plugin ships none and fetches nothing) into the same dictionary folder; after a join that dropped a hyphen, a dialog offers to check exactly those words, names the ones the dictionary does not know, and puts the hyphen back where you say so. The file is read in pieces and only until every word is answered, so an 8.5 MB dictionary costs a third of a second and nothing stays in memory.

**Language.** The plugin speaks English. One file next to it — `localization.json` in its own folder — replaces that text with your language; there is nothing to choose and nothing to configure, and anything the file leaves out stays English, key by key. Put the file there and reload the plugin. [`locales/english.json`](locales/english.json) is the whole plugin in English, ready to translate over, and [`locales/uk.json`](locales/uk.json) is a finished Ukrainian one. [docs/localization.md](docs/localization.md) has the steps.

**Format and Compress.** The editor's context menu formats what it can without anything installed: JSON with the plugin's own formatter (comments and key order kept), and indentation for every language whose CodeMirror mode provides it. Format and Compress sit in a **Format ▸** group, and a language nothing can format has no group at all. Full formatting is prettier, which the plugin neither bundles nor downloads: the repository carries prettier's own files in [`formatters/prettier/`](formatters) (unversioned folder, version in its `manifest.json`), plus ten community plugins bundled into one file each in [`formatters/extra/`](formatters) for PHP, XML, SQL, Svelte, Liquid, Jinja, Gherkin, nginx, `.properties` and INI. You copy the ones for your languages into `<vault>/.obsidian/plugins/native-file-editor/formatters/`, and the plugin reads and evaluates them only when Format is pressed — after checking the SHA-256 of the exact text it is about to run. A language whose file is missing or broken keeps its Format row and explains what to copy. [`formatters/README.md`](formatters/README.md) is the table of languages and the instruction; [docs/ADR-005](docs/ADR-005-formatter-from-the-plugin-folder.md) explains the trust boundary.
